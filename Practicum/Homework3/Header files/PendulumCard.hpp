...
public:
	// Constructors
	PendulumCard();
	PendulumCard(std::string name, std::string effect, unsigned int attackPoints, unsigned int defencePoints, short pendelumScale, Type type);
	...
	// Operators
	friend std::ostream& operator<<(std::ostream& stream, PendulumCard& card);
